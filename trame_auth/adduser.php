...
foreach (['roleav','nom', 'prenom', 'login', 'mdp', 'mdp2'] as $name) {
    if (empty($_POST[$name])) {
...
if(isset($_POST['roleav'])){
   // ont vérifie que une des deux input "radio" soit remplie
   if(empty($_POST['acheteur']) && empty($_POST['vendeur'])){
      //ici ta requête
       $error .="Role non sélectionné";
   }
}
...
foreach (['nom', 'prenom', 'login', 'mdp','roleav'] as $name) {
...
try {
    $SQL = "INSERT INTO users(nom,prenom,login,mdp,role) VALUES (:nom,:prenom,:login,:mdp,:roleav)";
    $stmt = $db->prepare($SQL);
